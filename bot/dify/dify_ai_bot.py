...
class DifyAIBot(Bot):
    ...
    def __init__(self):
        super().__init__()
        self.sessions = DifyAISessionManager(DifyAISession, model=conf().get("model") or "gpt-3.5-turbo")
        self.args = {}
    ...
    def reply(self, query, context: Context = None) -> Reply:
        if context.type == ContextType.TEXT:
            ...
            if not conf().get("text_to_image"):
                logger.warn("[DifyAI] text_to_image is not enabled, ignore the IMAGE_CREATE request")
    ...
    def _chat(self, query, context, retry_count=0) -> Reply:
        ...
        try:
            ...
            if context.get("generate_breaked_by"):
                logger.info(f"[DifyAI] won't set appcode because a plugin ({context['generate_breaked_by']}) affected the context")
                app_code = None
            else:
            ...
            logger.debug(f"[DifyAI] session={session_message}, session_id={session_id}")
            ...
            try:
                from DifyAI import DifyAIClient
                client_id = DifyAIClient.fetch_client_id()
                if client_id:
            ...
            logger.info(f"[DifyAI] query={query}, app_code={app_code}, model={body.get('model')}, file_id={file_id}")
            ...
            if res.status_code == 200:
                ...
                if res_code == 429:
                    logger.warn(f"[DifyAI] 用户访问超出限流配置，sender_id={body.get('sender_id')}")
                else:
    ...
    def _process_image_msg(self, app_code: str, session_id: str, query:str, img_cache: dict):
        try:
            ...
            if not app_info:
                logger.debug(f"[DifyAI] not found app, can't process images, app_code={app_code}")
                return None
            ...
            msg.prepare()
            logger.info(f"[DifyAI] query with images, path={path}")
    ...
    def _find_group_mapping_code(self, context):
        try:
            if context.kwargs.get("isgroup"):
                ...
                if config.plugin_config and config.plugin_config.get("DifyAI"):
                    DifyAI_config = config.plugin_config.get("DifyAI")
                    group_mapping = DifyAI_config.get("group_app_map")
    ...
    def reply_text(self, session: ChatGPTSession, app_code="", retry_count=0) -> dict:
        ...
        try:
            ...
            headers = {"Authorization": "Bearer " + conf().get("DifyAI_api_key")}

            # do http request
            base_url = conf().get("DifyAI_api_base", "https://api.link-ai.tech")
            ...
            if res.status_code == 200:
                ...
                reply_content = response["choices"][0]["message"]["content"]
                total_tokens = response["usage"]["total_tokens"]
                logger.info(f"[DifyAI] reply={reply_content}, total_tokens={total_tokens}")
                return {
    ...
    def _fetch_app_info(self, app_code: str):
        ...
        if res.status_code == 200:
            ...
            logger.warning(f"[DifyAI] find app info exception, res={res}")
    ...
    def create_img(self, query, retry_count=0, api_key=None):
        try:
            ...
            headers = {
                ...
                "Authorization": f"Bearer {conf().get('DifyAI_api_key')}"
            }
            data = {
                "prompt": query,
                "n": 1,
            ...
            url = conf().get("DifyAI_api_base", "https://api.link-ai.tech") + "/v1/images/generations"
    ...
    def _fetch_knowledge_search_suffix(self, response) -> str:
        try:
            if response.get("knowledge_base"):
                ...
                logger.info(f"[DifyAI] knowledge base, search_hit={search_hit}, first_similarity={first_similarity}")
                plugin_config = pconf("DifyAI")
...
class DifyAISessionManager(SessionManager):
    ...
    def session_reply(self, reply, session_id, total_tokens=None, query=None):
        ...
        try:
            ...
            logger.debug(f"[DifyAI] chat history, before tokens={total_tokens}, now tokens={tokens_cnt}")
        except Exception as e:
            logger.warning("Exception when counting tokens precisely for session: {}".format(str(e)))
        return session


class DifyAISession(ChatGPTSession):
